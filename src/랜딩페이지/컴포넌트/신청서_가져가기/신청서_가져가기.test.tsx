...
describe('신청서_가져가기 컴포넌트는', () => {
  context('크롬 브라우저일 때', () => {
    it('"이미지로 다운로드", "이미지를 클립보드로 가져가기"를 랜더링합니다.', () => {
      render(<신청서_가져가기 isChrome />);

      fireEvent.click(screen.getByRole('button'));

      expect(screen.getByText('이미지로 다운로드')).toBeInTheDocument();
      expect(screen.getByText('이미지를 클립보드로 가져가기')).toBeInTheDocument();
    });
  });

  context('크롬 브라우저가 아닐 때', () => {
    it('"이미지로 다운로드", "이미지를 클립보드로 가져가기"를 랜더링하지 않습니다.', () => {
      render(<신청서_가져가기 isChrome={false} />);

      fireEvent.click(screen.getByRole('button'));

      expect(screen.queryByText('이미지로 다운로드')).not.toBeInTheDocument();
